...
NUM_GAMES = 120
SEQ_LENGTH = 20  # I can feel this is going to be tricky. We are back to len 10.
TRAINING = True
TRAINING_ITER = 4  # If at 0, check the file_path manually
DENSE_NEURONS = 64
RNN_UNITS = 1024
EPOCH_NUM = 2
BATCH_SIZE = 1
...
if TRAINING_ITER == 0:
    ...
    file_path = f'db1_nn_abbey_n2000_0{TRAINING_ITER-1}.csv'  # Raw data
...
def build_model(seq_l, batch_size, vocab_size, rnn_units, dense_neurons):
    this_model = tf.keras.Sequential([
        # tf.keras.layers.Input(shape=(seq_l, vocab_size), dtype='int32'),
        tf.keras.layers.Input(batch_input_shape=[batch_size, seq_l, vocab_size], dtype='int32'),
...
if __name__ == "__main__":
    ...
    model = build_model(SEQ_LENGTH, BATCH_SIZE, 6, RNN_UNITS, DENSE_NEURONS)
    ...
    if TRAINING:
        ...
        model.fit(x1_seq, y1_seq, epochs=EPOCH_NUM, shuffle=False, batch_size=BATCH_SIZE, verbose=2)
